...
(function (global) {
   ...
   *        Cards.renderCognitiveSection();   // #cognitive-grid に描画
   *        Cards.renderTypesSection();       // .types 内 data-types-grid ごとに描画
   *      });
   *
   *  - function.html:
   *      document.addEventListener('DOMContentLoaded', () => {
   *        Cards.renderSingleFunctionFromQuery({
   *          target: '#cognitive-single',    // 任意のコンテナ
   *          param: 'code'                  // ?code=Ne など
   *        });
   *      });
   *
   *  - type.html:
   *      document.addEventListener('DOMContentLoaded', () => {
   *        Cards.renderSingleTypeFromQuery({
   *          target: '#type-single',
   *          param: 'type'                  // ?type=INTJ など
   *        });
   *      });
   */
  ...
  function createTypeCardElement(type, functionMap) {
    ...
    if (Array.isArray(type.stack)) {
      type.stack.forEach((st) => {
        ...
        li.innerHTML = `<strong>${st.slot}:</strong> ${fn ? fn.code : st.code}${fn ? ` (${fn.nameJa || ''})` : ''}`;
        ul.appendChild(li);
      });
    }
...
(function () {
  ...
  function autoInit() {
    ...
    if (!Cards) {
      console.error('[Cards] window.Cards not found (card-renderer.js の初期化前に autoInit が呼ばれています)');
      return;
    }
    ...
    switch (page) {
      ...
      case 'type': {
        ...
        // if (detail) {
        //   Cards.initTypeDetail({ target: detail });
        // }
        break;
      }

      case 'finder': {
        // finder.html 用の初期化（必要になったらここに）
        break;
      }
